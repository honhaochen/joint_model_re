...
class F1_triplet(F1_abc):
    ...
    def __call__(self, predictions: List[List[Dict[str, str]]],
        ...
        for g, p in zip(gold_labels, predictions):
            g_set = set('_'.join(('_'.join(gg['object']), gg['predicate'],
                                    '_'.join(gg['subject']))) for gg in g)
            p_set = set('_'.join(('_'.join(pp['object']), pp['predicate'],
                                    '_'.join(pp['subject']))) for pp in p)
