...
class Selection_Dataset(Dataset):
    def __init__(self, hyper, dataset):
        ...
        self.char_vocab = json.load(
            open(os.path.join(self.data_root, 'char_vocab.json'), 'r'))
        ...
        self.max_sentence_len = max(sentence_length_list)
        self.max_token_len = max(token_length_list)
    ...
    def __getitem__(self, index):
        ...
        char_tokens_id = self.text2char_tensor(text)

        return tokens_id, bio_id, selection_id, len(text), spo, text, bio, char_tokens_id
    ...
    def text2char_tensor(self, text: List[str]) -> List[torch.tensor]:
        oov = self.char_vocab['oov']
        sentence_list = []
        for token in text:
            char_list = list(map(lambda x: self.char_vocab.get(x, oov), list(token)))
            char_list.extend([self.char_vocab['<pad>']] * (self.max_token_len - len(list(token))))
            sentence_list.append(torch.tensor(char_list))
        
        sentence_list.extend([torch.zeros(self.max_token_len, dtype=int)] * (self.hyper.max_text_len - len(text)))
        return torch.stack(sentence_list)

    def bio2tensor(self, bio):
        # this loop is due to untidy data from scienceie raw data
...
class Batch_reader(object):
    def __init__(self, data):
        ...
        # tokens_id, bio_id, selection_id, spo, text, bio, char_tokens_id
